...
def extract_features(model, dataset, device, custom_desc='', batch_size=16):
    ...
    with torch.no_grad():
        for imgs, _ in tqdm(loader, desc=custom_desc, leave=False):
            imgs = imgs.to(device)
            features = model(imgs, inference=True)
...
class FederatedClient:
    def __init__(self, client_id, train_df, config, prototype_backbone=None):
        ...
        self.local_current_model = ReIDModel(backbone, head).to(self.cpu_device)
        ...
        # For Model-Contrastive Federated Learning
        self.prev_local_dict = None

    def train(self, server_msg):
        self.local_current_model.to(self.device)

        optimizer = optim.AdamW(
            self.local_current_model.parameters(),
            lr=self.config['lr'],
        ...
        # Local Model to train (R_{w_i^t})
        self.local_current_model.backbone.load_state_dict(global_weights, strict=True)
        self.local_current_model.train()
        
        global_model = None
        prev_local_model = None
        if self.prev_local_dict is not None:
            # Current Global Model (R_{w^t} = z_glob)
            global_model = copy.deepcopy(self.local_current_model).to(self.device)
            global_model.eval()

            # Previous Local Model (R_{w^{t-1}_i} = z_prev)
            prev_backbone = copy.deepcopy(self.local_current_model.backbone) 
            prev_backbone.load_state_dict(self.prev_local_dict, strict=True)

            prev_local_model = ReIDModel(prev_backbone, nn.Identity()).to(self.device)
        ...
        for epoch in range(self.config['local_epochs']):
            ...
            for imgs, labels in pbar:
                ...
                with autocast_mode.autocast(device_type='cuda'):
                    logits, proj_local = self.local_current_model(imgs, labels)
                    loss_cls = criterion(logits, labels)

                    contrastive_loss = torch.tensor(0.0, device=self.device)
                    if self.prev_local_dict is not None:
                        # Get embeddings from global and previous local models
                        with torch.no_grad():
                            proj_global = global_model(imgs)
                            proj_prev = prev_local_model(imgs)

                        # Compute contrastive loss
                        sim_pos = F.cosine_similarity(proj_local, proj_global, dim=1) / temperature
                        sim_neg = F.cosine_similarity(proj_local, proj_prev, dim=1) / temperature

                        logits_contrastive = torch.stack([sim_pos, sim_neg], dim=1)
                        labels_contrastive = torch.zeros(batch_size, dtype=torch.long, device=self.device)
                        contrastive_loss = criterion(logits_contrastive, labels_contrastive)

                epoch_contrastive_loss += contrastive_loss.item() * batch_size

                weighted_contrastive = self.config['contrastive_lambda'] * contrastive_loss.item()
                total_loss = loss_cls + weighted_contrastive

                epoch_loss += total_loss.item() * batch_size
                
                optimizer.zero_grad(set_to_none=True)
                scaler.scale(total_loss).backward()
                torch.nn.utils.clip_grad_norm_(self.local_current_model.parameters(), max_norm=1.0)
                scaler.step(optimizer)
                scaler.update()
        
        # Update previous global weights for next round
        self.prev_local_dict = copy.deepcopy(self.local_current_model.backbone.state_dict())
        self.prev_local_dict = {
            k: v.to(self.cpu_device) if isinstance(v, torch.Tensor) else v 
            for k, v in self.prev_local_dict.items()
        }

        self.local_current_model.to(self.cpu_device)
        ...
        return {
            ...
            'model_state': self.local_current_model.backbone.state_dict(),
            'num_samples': dataset_len,
            'loss': epoch_loss / dataset_len,
            'contrastive_loss': (epoch_contrastive_loss / dataset_len) * self.config['contrastive_lambda'],
        }

    def evaluate(self, gallery_set, query_set, mAP_at=[1,5]):
        eval_model = ReIDModel(self.local_current_model.backbone, nn.Identity()).to(self.device)
...
class FederatedServer:
    def __init__(self, config):
        ...
        self.global_backbone = ConvNeXtBackbone(embedding_dim=config['embedding_dim'], pretrained=True)
...
def main(config):
    ...
    for round_idx in range(1, config['rounds'] + 1):
        ...
        for client in clients:
            ...
            print(f"[Client {client.client_id}] Training Loss Total/Contrastive: {results['loss']:.4f}/{results['contrastive_loss']:.4f}, Time: {client_elapsed/60:.2f} minutes")
        ...
        if global_val_r1 > best_val_rank1:
            ...
            for client in clients:
                torch.save(client.local_current_model.state_dict(), results_path / f'best_backbone_client{client.client_id}.pth')
    ...
    for client in clients:
        ...
        client.local_current_model.load_state_dict(best_state)
...
if __name__ == "__main__":
    ...
    experiments = [
        {
            'results_name': 'FEDMOON_CLOSED_SET_HEAD',
            'body_part': 'head',
            'set': 'closed',
            'seeds': [42]
        },
