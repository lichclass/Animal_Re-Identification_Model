...
class FederatedClient:
    ...
    def train(self, global_backbone_state, global_prototypes, current_lr):
        ...
        # 1. PREPARE GLOBAL PROTOTYPES AS A MATRIX
        proto_matrix = None
        proto_id_to_idx = {}
        
        if global_prototypes:
            # Sort keys to ensure consistent indexing
            sorted_ids = sorted(global_prototypes.keys())
            # Stack all prototypes into [Num_IDs, Embedding_Dim]
            proto_matrix = torch.stack([global_prototypes[pid] for pid in sorted_ids]).to(self.device)
            # Map identity string to its index in the matrix
            proto_id_to_idx = {pid: i for i, pid in enumerate(sorted_ids)}
            
            # Ensure prototypes are normalized for cosine similarity
        ...
        temp = self.args.get('proto_temp', 0.07) 
        
        for epoch in range(self.args['local_epochs']):
            epoch_loss, epoch_cls_loss, epoch_proto_loss = 0.0, 0.0, 0.0
            iterator = tqdm(loader, desc=f'Client {self.client_id} | Epoch {epoch+1}')
            ...
            for images, train_labels, _, _ in iterator:
                ...
                with autocast_mode.autocast(device_type='cuda'):
                    ...
                    # 2. CONTRASTIVE PROTOTYPE LOSS
                    loss_proto = torch.tensor(0.0, device=self.device)
                    if proto_matrix is not None:
                        # Normalize local embeddings: [Batch, Dim]
                        emb_norm = F.normalize(emb, p=2, dim=1)
                        
                        # Compute Cosine Similarities: [Batch, Num_Global_IDs]
                        # This tells us how much each image looks like EVERY turtle in the database
                        sim_matrix = torch.matmul(emb_norm, proto_matrix.T) / temp
                        
                        # Create targets: map local labels back to global matrix indices
                        proto_targets = []
                        valid_mask = []
                        for local_idx in train_labels:
                            id_str = self.inv_local_id_map[local_idx.item()]
                            if id_str in proto_id_to_idx:
                                proto_targets.append(proto_id_to_idx[id_str])
                                valid_mask.append(True)
                            else:
                                valid_mask.append(False)
                        
                        if any(valid_mask):
                            proto_targets = torch.tensor(proto_targets, device=self.device)
                            loss_proto = criterion(sim_matrix[valid_mask], proto_targets) * self.args['lambda_proto']
