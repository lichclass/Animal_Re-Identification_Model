...
@torch.no_grad()
def compute_rank1_rank5_map(query_emb, query_lab, gallery_emb, gallery_lab, chunk=256):
    query_emb = query_emb.float().cpu()
    gallery_emb = gallery_emb.float().cpu()
    ...
    # chunked to avoid huge peak memory and argsort pressure
    for start in range(0, query_emb.size(0), chunk):
        qe = query_emb[start:start+chunk]
        ql = query_lab[start:start+chunk]

        sim = qe @ gallery_emb.t()               # CPU
        sorted_idx = torch.argsort(sim, dim=1, descending=True)
        sorted_gallery_labels = gallery_lab[sorted_idx]

        for i in range(sim.size(0)):
            q = ql[i]
            rel = (sorted_gallery_labels[i] == q)
            ...
            cumsum_rel = rel.cumsum(0).float()
            idx = torch.arange(1, rel.numel() + 1).float()
            precision_at_k = cumsum_rel / idx
            ap_sum += float((precision_at_k * rel.float()).sum().item() / num_rel)
...
def main():
    ...
    train_loader = DataLoader(
        train_set,
        # batch_sampler=pk_batch_sampler,            # ### CHANGED (PK SAMPLER)
